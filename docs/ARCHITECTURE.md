...
**Pure data + build script.** No runtime code. The frontend imports from `content/dist/vocab.json` (or a TS module), which is regenerated by `npm run build:content`. Same script runs in CI on `postinstall` if you want.
